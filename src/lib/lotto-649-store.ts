/**
 * Lotto 6/49 Data Store — Vercel-compatible
 *
...
const BCLC_URL = 'https://www.playnow.com/resources/documents/downloadable-numbers/649.zip';
...
async function loadFromHTTP(): Promise<Lotto649Draw[] | null> {
  try {
    const baseUrl = getBaseUrl();
    const res = await fetch(`${baseUrl}/lotto649-data.json`, {
      signal: AbortSignal.timeout(5000),
    });
    if (!res.ok) return null;
    const data = await res.json();
    return Array.isArray(data) ? data : (data.allMainDraws || null);
  } catch (e) {
    console.log('[649-store] HTTP load failed:', String(e).substring(0, 80));
    return null;
  }
}

function parseCSVLine(line: string): Lotto649Draw | null {
...
async function fetchFromBCLC(timeout = 8000): Promise<Lotto649Draw[]> {
  const response = await fetch(BCLC_URL, {
    headers: { 'User-Agent': 'Lotto649Dashboard/1.0' },
  ...
  try {
    ...
    for (const e of zip.getEntries()) {
      if (e.entryName.endsWith('.csv')) { csv = zip.readAsText(e); break; }
    }
    if (!csv) {
      for (const e of zip.getEntries()) {
        if (!e.isDirectory) {
          const t = zip.readAsText(e);
          if (t.includes('"649"') || t.includes('LOTTO 6/49')) { csv = t; break; }
        }
      }
    }
  } catch (zipErr) {
    console.error('[649-store] adm-zip failed:', String(zipErr).substring(0, 120));
    throw new Error('ZIP parse failed: ' + String(zipErr).substring(0, 60));
  }

  if (!csv) throw new Error('No CSV found in ZIP');
  const draws: Lotto649Draw[] = [];
 ...
 */
export async function getAll649Draws(forceRefresh = false): Promise<Lotto649Draw[]> {
  // 1. Try BCLC live data
  try {
    return await fetchFromBCLC(forceRefresh ? 15000 : 8000);
  } catch (e) {
    console.log('[649-store] BCLC unavailable:', String(e).substring(0, 80));
  }

  // 2. Fallback: static JSON via HTTP (works on Vercel!)
  const httpData = await loadFromHTTP();
  if (httpData && httpData.length > 0) {
    console.log('[649-store] Using static JSON fallback:', httpData.length, 'draws');
    return httpData;
  }
  ...
  try {
    ...
    console.error('[649-store] All sources failed');
